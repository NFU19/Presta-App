...
import { Usuario, RegistroUsuarioData } from '../types/usuario';
...
export const registrarUsuario = async (
  ...
  try {
    ...
    const matriculaExiste = await verificarMatriculaExistente(datosUsuario.matricula);
    if (matriculaExiste) {
    ...
    const message = error instanceof Error ? error.message : 'No pudimos registrar el usuario';
 ...
 */
export const obtenerUsuarioPorId = async (usuarioId: string): Promise<Usuario | null> => {
  try {
...
export const actualizarUsuario = async (
  ...
  try {
    ...
    if (datos.correo) {
      const correoExiste = await verificarCorreoExistente(datos.correo, usuarioId);
      if (correoExiste) {
    ...
    if (datos.matricula) {
      const matriculaExiste = await verificarMatriculaExistente(datos.matricula, usuarioId);
      if (matriculaExiste) {
